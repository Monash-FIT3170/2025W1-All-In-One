...
Meteor.methods({
  ...
  console.log('ten_fn type:', typeof updateData.ten_fn, 'value:', updateData.ten_fn);
  console.log('ten_ln type:', typeof updateData.ten_ln, 'value:', updateData.ten_ln);
  console.log('ten_pn type:', typeof updateData.ten_pn, 'value:', updateData.ten_pn);
  ...
  async 'tenantAddresses.remove'(addressId) {
    check(addressId, String);
    console.log(`[METHOD] tenantAddresses.remove called for address_id: ${addressId}`);
  ...
  return await Ten_SettingsEmployment.insertAsync(employmentData);
},
  ...
  async 'tenantEmployment.update'(employmentId, updates) {
    ...
    const allowedFields = ['emp_type', 'emp_comp', 'emp_job_title', 'emp_start_date', 'emp_verification'];

    const sanitizedUpdate = Object.fromEntries(
    Object.entries(updates).filter(([key]) => allowedFields.includes(key))
  );

  const existing = await Ten_SettingsEmployment.findOneAsync({ employment_id: employmentId });
  if (!existing) throw new Meteor.Error('not-found', 'Employment record not found');

  console.log(`[METHOD] tenantEmployment.update called for employment_id: ${employmentId}`, sanitizedUpdate);
  return await Ten_SettingsEmployment.updateAsync({ employment_id: employmentId }, { $set: sanitizedUpdate });
  }
});
