...
Meteor.methods({
  ...
  // ownership check
  const existing = await Ten_SettingsEmployment.findOneAsync({ employment_id: employmentId, ten_id: this.userId });
  if (!existing) throw new Meteor.Error('not-found-or-unauthorized', 'Employment record not found or unauthorized');
