...
Meteor.methods({
  ...
  async addProperty({
    ...
    try {
      ...
      const landlord = await Meteor.users.findOneAsync({
      ...
      if (!landlord) {
        throw new Meteor.Error(
          "landlord-not-found",
          "No landlord found with that email"
        );
      }
      ...
      const collectionSize =
        (await Properties.find({}, { fields: { prop_id: 1 } }).countAsync()) +
        1; // Counts the number of items in the collection, then adds 1
