...
export default function BasicLeases() {
  ...
  const leasedProperties = useTracker(() => {
    ...
    const isLoading = !propertiesHandle.ready() || !photosHandle.ready();
    if (isLoading) return [];
    ...
    // Get all leased properties for this tenant
    const properties = Properties.find({ leasedTo: userId }).fetch();

    // const allPhotos= Photos.find({prop_id:{$in: approvedPropIds}}).fetch();

    // Get photos for those properties
    const propIds = properties.map((p) => p.prop_id);
    const allPhotos = Photos.find({ prop_id: { $in: propIds } }).fetch();

    return properties.map((property) => {
      const photo = allPhotos.find(
        (p) => p.prop_id === property.prop_id && p.photo_order === 1
      );
      return {
        id: property.prop_id,
  ...
  // Meteor.call("tenantAccept", rentalAppId, (err) => {
  //   if (err) {
  //     alert(err.reason);
  //   } else {
  //     alert("You have accepted the lease!");
  //   }
  // });
