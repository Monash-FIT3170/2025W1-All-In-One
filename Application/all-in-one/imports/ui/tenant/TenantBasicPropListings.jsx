...
import { Properties, StarredProperties, Tenants } from "../../api/database/collections";
...
export default function TenantBasicPropListings() {
  ...
  const { isReady, properties, starredProperties } = useTracker(() => {
    const subProps = Meteor.subscribe("properties");
    const subStarred = Meteor.subscribe("starredProperties");
    const subTenants = Meteor.subscribe("tenants");

    const isReady = subProps.ready() && subStarred.ready() && subTenants.ready();

    const properties = isReady ? Properties.find().fetch() : [];
    const tenant = Tenants.findOne({ ten_id: Meteor.userId() });
    const starredProperties = isReady && tenant ? StarredProperties.find({ ten_id: tenant.ten_id }).fetch() : [];

    return { isReady, properties, starredProperties };
  ...
  const availableProperties = properties.filter((p) => p.prop_status === "Available");
