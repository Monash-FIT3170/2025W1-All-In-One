...
import { Ten_SettingsIncomes } from '/imports/api/database/collections';
  ...
  return Ten_SettingsIncomes.find().fetch();
    ...
    const handleAddIncome= async (incomeData) => {
        ...
        if (editingIncome){
            ...
            Meteor.call ('tenantIncomes.update', editingIncome.inc_id, updateData, (err) =>{
                setStatusMessage(
                    err ? `Error updating income: ${err.message}` : 'Income updated successfully'
                );
            });
        } else {
            // inserting new one
            ...
            Meteor.call('tenantIncomes.insert', newIncome, (err) => {
                setStatusMessage(
                    err ? `Error saving income: $(err.message)`: 'Income added successfully'
                );
            });
        }
    ...
    if (window.confirm('Are you sure you want to delete this income document?')) {
      Meteor.call('tenantIncomes.remove', incId, (err) => {
